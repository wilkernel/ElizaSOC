...
echo "1⃣ Verificando serviço Suricata..."
...
# 2. Verificar jq
echo "2⃣ Verificando dependências..."
...
echo "3⃣ Verificando diretório de logs..."
...
if [ -d "$LOG_DIR" ]; then
    ...
    if [ -r "$LOG_DIR" ]; then
        ...
        echo "  Sem permissão de leitura no diretório"
...
echo "4⃣ Verificando arquivo eve.json..."
...
if [ -f "$EVE_JSON" ]; then
    ...
    if [ -r "$EVE_JSON" ]; then
        ...
        echo "  Arquivo existe mas sem permissão de leitura"
...
echo "5⃣ Verificando configuração do Suricata..."
if [ -r "/etc/suricata/suricata.yaml" ]; then
    if grep -q "eve-log" /etc/suricata/suricata.yaml 2>/dev/null; then
        ...
        echo "  eve-log não encontrado na configuração"
    fi
else
    echo "  Não foi possível ler /etc/suricata/suricata.yaml"
fi
echo ""

# 6. Verificar logs do sistema
echo "6⃣ Verificando últimos erros do Suricata..."
