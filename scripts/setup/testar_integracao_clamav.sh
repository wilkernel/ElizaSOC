...
echo "1⃣ Criando diretório de quarentena..."
...
echo "2⃣ Criando arquivo de teste EICAR..."
...
echo "3⃣ Testando escaneamento direto com ClamAV..."
...
echo "Testando módulo Python clamav_scanner.py..."
cd "$SCRIPT_DIR/../.."
...
if echo "$result" | grep -q '"infected": true'; then
	...
	if echo "$result" | grep -q '"quarantined": true'; then
		...
		echo "  Arquivo não foi colocado em quarentena (pode ser permissão)"
	fi
else
...
if curl -s http://localhost:5000/api/viruses > /dev/null 2>&1; then
	...
	if echo "$scan_result" | grep -q '"infected"'; then
		...
		echo "  API retornou: $scan_result"
	fi
else
	echo "  Dashboard não está rodando (inicie com: bash iniciar_dashboard.sh)"
fi
echo ""

# Limpar
echo "6⃣ Limpando arquivos de teste..."
rm -f "$TEST_FILE"
